...
for f = [3, 5:15, 18]
    ...
    driftNum = regexp(dasbrList(f).name, '\d+', 'match');
    ...
    text(dasbr.Longitude(1)-0.05, dasbr.Latitude(1)+0.05, ['d' driftNum{1}], ...
        'FontSize', 10, 'Color', 'white')
    text(dasbr.Longitude(end)-0.05, dasbr.Latitude(end)+0.05, ['d' driftNum{1}], ...
        'FontSize', 10, 'Color', 'white')
    % dasbrs(f).Number = f;
...
% set axes limits
% xlim(CONFIG.map.lonLim)
% ylim(CONFIG.map.latLim)
xlim([-128.5 -123.4])
ylim([40.4 45])
...
set(gcf, 'Position', [50 100 940 940]);
...
exportgraphics(gcf, fullfile(path_repo, 'maps', ...
    sprintf('DASBRs_and_gliders_coloredByTime_%s.png', ...
    datetime('now', 'Format', 'uuuu-MM-dd'))), ...
    'Resolution', 300);
